        ...
        <div class="col-md-12">
            <div class="ibox ">
                ...
                <div class="ibox-content">
                    <form action="{{ route('admin.core.nav.store') }}" method="POST">
                        @csrf
                        <div class="row">
                            <div class="col-md-8">
                            ...
                            <div class="col-md-4">
                                <div class="form-group">
                                    ...
                                    <select name="group" id="group" class="form-control">
                                        ...
                                        <option value="{{ \Modules\core\models\Navigation::GROUP_HEADER }}" {{ old('group') == \Modules\core\models\Navigation::GROUP_HEADER ? 'selected' : '' }}>{{ __('Header') }}</option>
                                        <option value="{{ \Modules\core\models\Navigation::GROUP_FOOTER }}" {{ old('group') == \Modules\core\models\Navigation::GROUP_FOOTER ? 'selected' : '' }}>{{ __('Footer') }}</option>
                                    </select>
                                </div>
                                    ...
                                    <select name="parent_id" id="parent" class="form-control">
                                        ...
                                        @if(!empty($navigations = \Modules\core\models\Navigation::getInstance()->buildNavTree()))
                                            @foreach($navigations as $navigation)
                                                <option value="{{ $navigation['navigation_id'] }}" {{ old('parent_id') == $navigation['navigation_id'] ? 'selected' : '' }}>{{ $navigation['title'] }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
